...
import SequenceCanvasToolbar from './SequenceCanvasToolbar';
...
const SequenceLayout: React.FC<SequenceLayoutProps> = ({ sequenceId }) => {
  ...
  const handleUndo = useCallback(() => {
    // 실행 취소 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 실행 취소');
  }, []);

  const handleRedo = useCallback(() => {
    // 다시 실행 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 다시 실행');
  }, []);
  ...
  const handleTogglePublic = useCallback(() => {
    ...
    console.log('시퀀스 다이어그램 public/private 토글:', !isPublic);
  }, [isPublic]);

  const handleSearch = useCallback(() => {
    setSearchActive(!searchActive);
    // 검색 패널 토글 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 검색');
  }, [searchActive]);

  const handleZoomToFit = useCallback(() => {
    // 한눈에보기 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 한눈에보기');
  }, []);

  const handleCopy = useCallback(() => {
    // 복사 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 복사');
  }, []);

  const handlePaste = useCallback(() => {
    // 붙여넣기 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 붙여넣기');
  }, []);

  const handleGrid = useCallback(() => {
    setShowGrid(!showGrid);
    // 그리드 토글 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 그리드 토글');
  }, [showGrid]);

  const handleHistory = useCallback(() => {
    setShowHistoryPanel(!showHistoryPanel);
    // 히스토리 패널 토글 로직 (나중에 구현)
    console.log('시퀀스 다이어그램 히스토리');
  }, [showHistoryPanel]);
    ...
    <Container $darkMode={darkMode}>
      <TopContainer $darkMode={darkMode}>
        ...
        <ToolboxContainer $darkMode={darkMode}>
          <SequenceToolbox darkMode={darkMode} />
        </ToolboxContainer>
